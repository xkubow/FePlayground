<template>
	<k-col :span="24" class="caption_eskalace">
		<span class="d-f ai-c">
			...
			<template v-if="!isUzavreta">
				...
				<k-button plain v-if="keSchvaleni" :disabled="!eskalaceCmd.operationEskalaceSchvaleni" type="success"
					@click="$emit('schvaleni')">{{ t('schvaleni') }}
				</k-button>
				<k-button plain v-if="keSchvaleni" type="danger" @click="$emit('zamitnuti')">{{ t('zamitnuti') }}
				</k-button>
				<template v-else>
					<k-button v-if="!schvalena" plain @click="$emit('predajKeSchvaleni')">{{ t('keSchvaleni') }}
					</k-button>
					<k-button v-if="schvalena && eskalaceCmd.eskalaceStatusEnum === EnumEskalaceStatus.Vyroba" plain
						@click="$emit('kAnalyze')">{{ t('kAnalyze') }}
					</k-button>
					<k-button v-if="schvalena && eskalaceCmd.eskalaceStatusEnum === EnumEskalaceStatus.VReseni"
						:disabled="!eskalaceCmd.operationEskalacePredani" plain @click="$emit('vraceniKAnalyze')">{{
						t('vraceniKAnalyze') }}
					</k-button>
					<k-button v-if="eskalaceCmd.eskalaceStatusEnum === EnumEskalaceStatus.Analyza" plain
						:disabled="!eskalaceCmd.operationEskalacePredani" @click="$emit('doVyroby')">{{
						t('predaniDoVyroby') }}
					</k-button>
					<k-button v-if="eskalaceCmd.eskalaceStatusEnum === EnumEskalaceStatus.Analyza" plain
						:disabled="!eskalaceCmd.operationEskalacePredani" @click="$emit('kReseni')">{{
						t('predaniKReseni') }}
					</k-button>
					<k-button plain :disabled="!uzavreniEnabled" @click="$emit('uzavreni')">{{ t('uzavreni') }}
					</k-button>
				</template>
				<k-button plain @click="dialogOpnCheckVisible = true">{{ t('OPN') }}</k-button>
				<k-button plain @click="createVraceniBaterie" v-if="!eskalace.vraceniBaterie">{{ t('VytvorVraceniBaterie')
					}}</k-button>
				<k-button plain @click="editVraceniBaterie" v-else>{{ t('BaterieVracena') }}</k-button>
			</template>
			<k-button v-else :disabled="!operationEskalaceZnovuotevreni" plain @click="$emit('otvoreniUzavrene')">{{
				t('ZnovuOtevreni') }} </k-button>
		</span>
	</k-col>
	<template v-if="!isUzavreta">
		<k-col>
			<k-select :span="8" label-key="ridiciJednotka" show-more v-model="eskalaceCmd.ridiciJednotkaId"
				:options="cacheStore.dropDownList.ridiciJednotka" filterable :clearable="false" />
		...
		<k-select :span="4" label-key="UzivateleSkupina" show-more v-model="eskalaceCmd.prirazenaSkupinaUzivateluId"
			v-bind="{ optionsValueName: 'id', optionsTextName: 'text' }" :options="priradeniSkupinaList" />
		<k-select :span="20" :label="t('uzivatel', 2)" v-model="eskalaceCmd.uzivatelPrirazeno" multiple filterable
			remote reserve-keyword :remote-method="predaniRemoteMethod" :loading="predaniLoading"
			:options="eskalaceCmd.uzivatelPrirazenoDropDownList" />
	</template>
	<template v-if="keSchvaleni">
		<k-select :span="4" label-key="SchvalovateleSkupina" show-more
			v-bind="{ optionsValueName: 'id', optionsTextName: 'text' }"
			v-model="eskalaceCmd.schvalujeSkupinaUzivateluId" :options="schvalovateleSkupinaList" />
		<k-select :span="20" label-key="schvalovatel" v-model="eskalaceCmd.uzivatelSchvaluje" multiple filterable remote
			reserve-keyword :remote-method="schvalovateleRemoteMethod" :loading="schvalovateleLoading"
	...
	<k-col>{{ `${t('text')}:` }}</k-col>
	<k-rich-text v-model="eskalaceCmd.text"
		v-bind="{ references: eskalace?.priloha?.rows ?? [], baseUrl, uploadVisible: false, editable: !isUzavreta }" />
