...
<template>
	<k-layout-list ref="listLayout" v-bind="{ operations: table?.operations, leaving, useCreate: false }" autoFilter v-on="layoutListyeners">
		...
		<template #header>
			...
			<k-select v-bind="{ options: verzeFilter(vyrobniLinkaId!), labelKey: 'verze', disabled: !vyrobniLinkaId, span: 12, wrapp: false }" v-model="verze" class='mw-6' />
			</k-row>
		</template>
		<template #content>
			<k-table
			...
			<pracoviste-edit
				...
				v-bind="{
					verze: verze!, vyrobniLinkaId: vyrobniLinkaId!, vyrobniLinkaKontrolniBodId: selectedRow?.vyrobniLinkaKontrolniBodId!, 
					vyrobniLinkaSegmentId: selectedRow?.vyrobniLinkaSegmentId!, kontrolniBodList, segmentList
				}"
				v-on="{ loadRows }"
			/>
		</template>
