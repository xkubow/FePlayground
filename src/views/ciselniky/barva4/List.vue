...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function cellStyle({ row, columnIndex }: { row: Barva4; columnIndex: number }) {
        return columnIndex === 1 ? { backgroundColor: converToHexaColor(row.uiBarvaKod) } : undefined;
      }

      async function rowDelete(row: Barva4) {
        await apiProvider.deleteEntity(row.barva4Kod);
        list.loadRows();
      }
