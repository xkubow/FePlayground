<template>
  <k-layout-list ref="listLayout" v-bind="{ operations: table?.operations, leaving }" v-on="layoutListyeners">
    ...
    <template #filter>
      <k-select label-key="modelovaTridaKod" v-model="modelovaTridaKod" :options="dropDownList.modelovaTridaKod" />
    </template>
    <template #content>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      async function rowDelete(row: ModelovaTrida) {
        await apiProvider.deleteEntity(row.modelovaTridaKod);
        list.loadRows();
      }
