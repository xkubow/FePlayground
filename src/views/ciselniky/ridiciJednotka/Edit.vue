<template>
  <k-layout-edit v-bind="{ leaving }" v-on="{ ...layoutListeners }">
    ...
    <template #actions />
    <k-card row>
      <k-input v-model.number="diagnostickaAdresa" label-key="diagnostickaAdresa" :suffixText="daHexa" :span="3" />
      <k-input v-model="ridiciJednotkaText" label-key="ridiciJednotkaText" uppercase :span="21" />
      <k-table-frame>
        <template #header>
        ...
        <k-table
          v-bind="{ ...tables.label, tableName: tables.label.name, objectName: `${LABEL_NAME}${NAME}`, rowKey: tables.label.rowKey }"
          v-on="{ rowDelete }"
        />
      </k-table-frame>
    </k-card>
...
<script lang="ts">
  ...
  import type { ro } from 'element-plus/es/locale/index.mjs';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      async function addLabel() {
        if (edit.isCreateMode.value) await edit.changeToEdit();
        openPage(createName(`${LABEL_NAME}${NAME}`), PageMode.CREATE, { kod: mapedLast.id.value });
      }

      const daHexa = computed(() => {
        var tt = (typeof mapedLast.diagnostickaAdresa.value == 'string' ? parseInt(mapedLast.diagnostickaAdresa.value) : mapedLast.diagnostickaAdresa.value)
          ?.toString(16)
          .toUpperCase();
        return tt;
      });
