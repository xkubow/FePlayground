      ...
      <tbody>
        <tr v-for="(d, i) in parsedRows" :key="d.rowNumber ?? i" class="detail-row">
          ...
          <td class="col-pracovisteVyskytuZavady">
            <div v-for="(vyskyt, i) in d.pracovisteVyskytuZavady" :key="i" :class="[vyskytClass(vyskyt.testIo, d.detail?.length > 0), 'vyskyt', 'p-5']">
              {{ vyskyt.text }}
            </div>
          </td>
...
<script setup lang="ts">
  ...
  function vyskytClass(isOk: boolean, hasDetail: boolean): string {
    if (hasDetail) return 'background_greyed_' + (isOk ? 'ok' : 'nok');
    else return 'background_test_' + (isOk ? 'opravene' : 'nok');
  }
