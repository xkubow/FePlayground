<template>
	<k-table-frame v-bind="{ paging: tableBinds.paging, totalCount: tableBinds.totalCount }"
		v-model.paging="tableBinds.paging">
		<k-table ref="mainTable" v-bind="tableBinds" @selection-change="mainSelectionChange" @row-click="rowClick"
			:expand-row-keys="expandRowKeys" @expand-change="onExpandChange">
			<template #ridiciJednotka="{ column }">
				<k-table-column
					v-bind="{ column, tableName: tableBinds.name, labelKey: column.i18nKey, filterable: false }"
			...
			<template #pblText="{ column }">
				<k-table-column
					...
					<template #default="{ row }">
						<div class="d-f ai-c">
							<k-table-cell v-bind="{ row: row, columnId: column.id, formatter: column.formatter }" />
							<el-icon class="ml-10 cursor-p" :size="15"
								@click="bugClicked($event, row)"><link-icon /></el-icon>
						</div>
					</template>
				</k-table-column>
			</template>
			...
			<template #detail="{ column }">
				<k-table-column
					v-bind="{ column, type: 'expand', tableName: tableBinds.name, labelKey: column.i18nKey, align: 'center' }">
					<template #default="{ row }">
						<div class="table-detail">
							<ChybyDetailTable v-if="isExpanded(row) && row.detail?.length"
								:diagnostickaAdresa="row.diagnostickaAdresa" :parentRow="row" :rows="row.detail"
								:selectable="tableBinds.selectable" :vyskytClass="vyskytClass"
								@toggleOne="toggleOneDetail" :openZavady="openZavady" :onBugClick="bugDetailClicked"
								:t="t" />
...
function onExpandChange(changed: { row: Row<RowChyby>, expandedRows: Row<RowChyby>[] }) {
	...
	if (expandRow)
		expandRowKeys.value.push(changed.row.rowNumber);
	else
		expandRowKeys.value = expandRowKeys.value.filter(rk => rk !== changed.row.rowNumber);
}
...
function testCellClass({ row, column, rowIndex, columnIndex }: { row: Row<RowChyby>; column: string; rowIndex: number; columnIndex: number }) {
	if (columnIndex === 0) {
		if (row.detail.every(p => p.detail.length > 0))
			return row.opravenoVse ? 'background_greyed_opravene' : 'background_greyed_nok';
...
function vyskytClass(hasDdetail: boolean, isErr: boolean) {
	let colorClass;
	if (hasDdetail)
		colorClass = isErr ? 'background_greyed_nok' : 'background_greyed_ok';
	else
		colorClass = isErr ? 'background_test_nok' : 'background_test_opravene';
