    ...
    <el-dialog :title="t('CasOpravy')" @close="$emit('update:modelValue', false)" @open="open">
        <div v-loading="loading" element-loading-text="Loading...">
            <k-row>
                ...
                <k-date-interval v-model="interval" label-key="Od-Do" v-bind="{ type: 'datetime', format: 'DD.MM.YYYY HH:mm' }" isHiddenLabel overwriteHiddenLabel :span=16 />
            </k-row>
            <k-row>
            ...
            <el-descriptions v-if="casOpravyVozidla.id">
                <el-descriptions-item :span="24" :label="`${t('Zavada')}:`">{{ casOpravyVozidla.sqsZavadaMistoText}} - {{ casOpravyVozidla.sqsZavadaTypText }} - {{ casOpravyVozidla.sqsZavadaVinikText }}</el-descriptions-item>
		        <el-descriptions-item :span="24" :label="`${t('vytvoril')}:`">{{ casOpravyVozidla.vstupUzivatelZobrazeneJmeno}} - {{ casOpravyVozidla.vstupDatum ? toLocale(casOpravyVozidla.vstupDatum) : '' }}</el-descriptions-item>
                <el-descriptions-item :span="24" :label="`${t('Editoval')}:`">{{ casOpravyVozidla.editUzivatelZobrazeneJmeno}} - {{ casOpravyVozidla.editDatum ? toLocale(casOpravyVozidla.editDatum) : '' }}</el-descriptions-item>
	        </el-descriptions>
        </div>
        <template #footer>
...
const dobaOpravyMinutyCmp = computed({
    ...
    set(val) {
        ...
        interval.value.od = dayjs(interval.value.do).subtract(val ?? 0, "minute").toDate();
...
async function open() {
    ...
    if (!!propsRef.casOpravyId.value) {
        ...
        if (response?.data) {
            ...
            interval.value.do = dayjs(interval.value.od).add(casOpravyVozidla.value.dobaOpravyMinuty ?? 0, "minute").toDate();
        }
    }
    loading.value = false;
}
...
async function save() {
    ...
    if (!!propsRef.casOpravyId.value && entity.id)
        await apiProviderCasOpravy.updateEntity(entity.id, entity);
    else
        await apiProviderCasOpravy.createEntity(entity);
