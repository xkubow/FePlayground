<template>
    <div class="bordered pl-10">
        <k-row>
            <k-input-number label-key="dobaOpravyMinuty" :modelValue="dobaOpravyMinutyCmp" :span=4 :required="required"
                @update:modelValue="setDobaOpravyMinuty" />
            <k-date-interval :modelValue="intervalCmp" label-key="Od-Do"
                v-bind="{ type: 'datetime', format: 'DD.MM.YYYY HH:mm', required: true }" isHiddenLabel
                overwriteHiddenLabel :span=16 @update:modelValue="setInterval" validationProperty="intervalOpravy" />
        </k-row>
        <k-row>
            <k-input label-key="text" validationProperty="casOpravyText" v-model="textCmp"
