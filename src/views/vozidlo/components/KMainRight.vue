<template>
	<k-col>
		<k-table-frame v-if="zavada" label-key="zadaneZavadyKVozu">
			<zavada-table v-bind="{ rows: zavada.rows, operations: zavada.operations, showSummary: true }"
				v-on="{ refresh: refreshVozidloZavadaTable }" />
		</k-table-frame>
	</k-col>
	<k-col>
	...
	<k-col>
		<k-table-frame v-if="sqsZavady" label-key="sqsZavadyKVozu">
			<k-table v-bind="{ columns: sqsZavady.columns, rows: sqsZavady.rows, tableName: sqsZavady.name, rowKey: sqsZavady.rowKey }">
				<template #status="{ column }">
					<k-table-column
						v-bind="{ tableName: sqsZavady.name, labelKey: column.i18nKey, column, prop: `${column.id}` }">
