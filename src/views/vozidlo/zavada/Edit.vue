<template>
  <k-layout-edit data-testid="zavada-form" v-bind="{ leaving, model: mappedLast, rules }" v-on="{ ...layoutListeners }" ref="layoutRef">
          ...
          <k-col :span="10">
            ...
            <div>
              <k-svg-check-box v-model="odeslanoDoSqsDatumCheck" :disabled="true" :size="20">
                <span :style="odeslanoDoSqsStatusColor">{{
                  `${odeslanoDoSqsDatumCheck ? '' : 'NE'}ODESLANO do
									SQS`
                }}</span>
                <span class="ml-10">{{ `${odeslanoDoSqsDatum?.toLocaleString(actualLocale) ?? ''}` }}</span>
                <k-button v-if="!odeslanoDoSqsDatumCheck && canCreate" @click="odesliDoSqs" class="ml-10" label-key="odeslatSQS" />
              </k-svg-check-box>
            </div>
          </k-col>
          <k-col :span="14">
            <CasOpravyFrame
              ...
              @update:dobaOpravyMinuty="updateDobaOpravyMinuty"
              @update:text="updateCasOpravyText"
            ...
            <k-row>
              ...
              <k-select
                ...
                :disabled="isFromNavrhovane || !sqsZavadaSkupinaId || isViewMode"
                filterable
              />
              <k-select
                :span="6"
              ...
              <k-select
                :span="6"
                label-key="mistoVznikuZavady"
                v-model="pracovisteVznikuZavadyArcId"
                filterable
                show-more
                :options="dropDownLists.pracovisteVznikuZavady"
              />
              <k-select
                :span="6"
                label-key="mistoZadaniZavady"
                v-model="pracovisteZadaniZavadyArcId"
                filterable
                show-more
                :options="dropDownLists.pracovisteZadaniZavady"
                @change="pracovisteChange"
              />
            </k-row>
          </k-col>
          <k-input :disabled="isViewMode" label-key="text" :span="10" v-model="text" :autosize="{ minRows: 2, maxRows: 4 }" type="textarea" />
          <k-input
            :disabled="!isCreateMode"
            label-key="navrhovaneReseni"
            :span="14"
            v-model="zpusobOpravy"
            :autosize="{ minRows: 2, maxRows: 4 }"
            type="textarea"
          />
          <k-col>
            <table-chyby
              :rows="tables.chyby.rows"
              :columns="tables.chyby.columns"
              :rjInfopRows="tables.ridiciJednotkaInfo.rows"
              @detail-select.once="detailSelect"
            />
          </k-col>
          <k-table-frame label-key="navrhovaneZavady">
            <NavrhovaneZavadyVue v-bind="{ rows: tables.navrhovane.rows, disabled: true, selectable: true }" />
          </k-table-frame>
          <k-col>
            <CasOpravy
              v-if="!isCreateMode && id"
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function cleanupElementPlusPoppers() {
        ...
        document.querySelectorAll('.el-popper, .el-select__popper, .el-select-dropdown, .el-tooltip__popper, .el-popover').forEach((n) => n.remove());

        // also remove any stray aria controls that can keep references
        document.querySelectorAll("[aria-controls^='el-id-']").forEach((n) => n.removeAttribute('aria-controls'));
      }
      ...
      function loadTables(filter: Record<string, unknown> | undefined, tablesToFetch: string[] | undefined) {
        if (edit.isCreateMode.value) {
          tablesToFetch = tablesToFetch?.filter((p) =>
            mappedLast.isFromNavrhovane.value ? p !== NAME_CAS_OPRAVY : p !== 'navrhovane' && p !== NAME_CAS_OPRAVY,
          );
        }
