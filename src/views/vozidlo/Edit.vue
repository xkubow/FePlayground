<template>
	<k-layout-edit v-bind="{ leaving, canDelete: false, canSave: false }" v-on="{ ...layoutListeners }">
		...
		<template #actions>
			<k-actions class="hidden-sm-and-down"
		...
		<k-card>
			<vozidlo-table
				...
				v-model:vozidloStitek="vozidloStitek" v-on="{ updateStitky, updateZavadaNV, updateBaterieVyjmuta }"
				highlight />
			<div class="mt-10">
				<el-tabs v-model="activeTabName">
					<el-tab-pane :label="t('hlavniUdaje')" name="HlavniUdaje">
						<k-row :gutter="10">
							...
							<k-col :xs="12" :sm="20" :md="19" :lg="21" :xl="22">
								<k-actions class="hidden-md-and-up"
									v-bind="{ aktualnyStatusBg, eskalaceExists: !!eskalace[0]?.knr, operationEskalaceCreate }"
									v-on="{ openTestyUps, createZavadu, createEskalaci, openPuvodniSqs, openPuvodniUps, openCarRfid, openElsa }" />
					...
					<el-tab-pane :label="t('komentare')" name="Komentare">
						<k-row :gutter=4>
							...
							<k-col :span=4>
								<div>{{ t('Prilohy') }}</div>
								<frame-prilohy v-model="tables.priloha.rows"
									v-bind="{ entita, parentId: knr, beforeRemove: beforeRemovePriloha, operationDeletePriloha }" />
							</k-col>
						</k-row>
					</el-tab-pane>
					<el-tab-pane :label="t('eskalace')" name="Eskalace">
						<div v-if="eskalace.length > 0">
							<el-pagination layout="prev, pager, next" v-model:current-page="currentEskalace"
								:total="eskalace.length" :page-size=1 :hide-on-single-page="true" />
							<k-row :gutter=4>
								<k-col :span=20>
									<div v-if="selectedEskalace?.knr" class="el-card p-0">
										<k-row :span=24>
											<eskalace-frame ref="eskalaceFrame" v-model:eskalace="selectedEskalace"
								...
								<k-col :span=4 v-if="selectedEskalace?.priloha?.rows">
									<div>{{ t('Prilohy') }}</div>
									<frame-prilohy v-model="selectedEskalace.priloha.rows" v-bind="{
					...
					<el-tab-pane :label="t('prInfo')" name="PrInfo">
						<pr-cislo-frame name="PrCislo" :rows="tables.prCislo.rows" :knr="knr" v-model:columns="tables.prCislo.columns"
							v-model:paging="tables.prCislo.paging" v-model:filter="tables.prCislo.filter"
							v-on="{ filterClick, loadRows }" />
					</el-tab-pane>
					<el-tab-pane :label="t('CasOpravy')" name="CasOpravy">
						<CasOpravy v-bind="{ casOpravy: tables.casOpravy, knr, showSummary: true }"
							@refresh="refreshCasOpravy" />
					</el-tab-pane>
				</el-tabs>
			</div>
		</k-card>
		<create-eskalace-dialog v-if="knr" v-model:visible="createEskalaceDialog.visible"
			v-model:eskalace="selectedEskalace" v-model:eskalaceEditDatum="eskalaceEditDatum" v-bind="{
				vozidloId: knr!,
...
export default defineComponent({
	...
	setup() {
		...
		onMounted(() => {
			...
			} else if (pageManagerStore.previous?.name.includes('eskalace')
				|| pageManagerStore.previous?.name.includes('vraceniBaterie')) {
				mapedLast.activeTabName.value = 'Eskalace';
			}
