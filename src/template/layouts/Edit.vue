<template>
  <k-container v-if="!leaving" class="full-content-height-container margin-content edit-container" direction="vertical">
    <k-header class="sub-page-header">
      <k-col>
        <k-row class="ai-c">
          <k-button
            data-testid="zavada-close"
            id="edit-layout-back-button"
            v-if="canClose"
            plain
            @click="$emit('closeClick', $event)"
            :disabled="false"
            :icon="CloseIcon"
          />
          <k-button v-if="canSave" :disabled="saveDisabled" plain type="success" @click="$emit('saveClick', $event)" :icon="CheckIcon" />
