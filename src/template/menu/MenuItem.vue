  ...
  <template v-for="(item, index) in menu" :key="index">
    <template v-if="item.menuType === undefined">
      <el-menu-item v-if="item.uriType === UriType.PATH" :index="`${index}`">
        <a class="menu-item" :href="getHrefRoute(item)">
          <el-icon v-if="collapsed && item.icon" class="menu-icon">
            <component :is="item.icon" />
          </el-icon>
          <span v-else>{{ menuText(item) }}</span>
        </a>
      </el-menu-item>
