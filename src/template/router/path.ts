...
export class Path implements RouteRecordSingleView {
  ...
  constructor(
    public name: string = '',
    components: RouteComponent | Components = EmptyRouterView,
  ) {
    this.path = name;
    this.name = name;
...
export function replaceEditPath(name: string, component: RouteComponent): RouteRecordRaw {
  return {
...
export function clearAllEditPath(name: string, component: RouteComponent): RouteRecordRaw {
  return {
    path: `:mode(${PageMode.EDIT})/:id?/:anchor?`,
    component,
