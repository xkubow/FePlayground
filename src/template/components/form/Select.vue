...
<template>
	<k-wrapper ref="wrapper" :show-label="!isHiddenLabel && showLabel"
		:disabled="isDisabled" :label="labelText" :validation-property="validationPropertyCmp" :span="span" :class="wrappClass" :wrapp="wrapp">
		<el-select :teleported="false" class="k-select" v-model="vmodel"
				v-bind="{ ...$props, ...$attrs, disabled: isDisabled }" :value-key="optionsValueName"
				:placeholder="placeholderText" @change="change" @clear="clear" @remove-tag="removeTag"
				@visible-change="visibleChange" v-on="{ focus }" :popper-class="popOverClassName">
			<slot>
				<el-option v-for="item in optionsExtendedFiltered" :key="item[optionsValueName]" :label="item[optionsTextName]"
