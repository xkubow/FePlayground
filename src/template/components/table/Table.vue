  ...
  >
    <template v-if="selectable">
      <slot name="selectable">
        <k-table-column-select
          v-model:selectAll="selectAll"
          :indeterminate="selectIndeterminate"
          :disabled="isDisabled"
          @selectChange="$emit('selection-change', $event)"
        />
      </slot>
    </template>
    ...
    <slot>
      <template v-if="isDefaultSlotEmpty">
        <slot v-for="column in columnsFiltered" :key="column.id" :name="column.id" :column="column" :nextSortIndex="nextSortIndex">
          <k-table-column
            :tableName="tableName"
            :labelKey="column.i18nKey"
            :column="column"
    ...
    <k-table-column-operations
      v-if="buttonColumnVisible"
      fixed="right"
      :labelKey="'operations'"
      ...
      :rowKey="rowKey"
      :useRowOperations="useRowOperations"
      @row-add-to="$emit('row-add-to', $event)"
