import { isNil, max, orderBy, sortBy } from 'lodash-es';
...
export function useTableSorting(
  ...
  const colorDescending = computed(() => (!isNil(column?.value?.sortingAscending) && !column?.value?.sortingAscending ? 'blue' : 'grey'));
  ...
  function sort(ascending?: boolean) {
    if (emits && column?.value) {
      ...
      } else sortingAscending = !isNil(column.value?.sortingAscending) && !column.value?.sortingAscending ? undefined : !column.value?.sortingAscending;
  ...
  const nextSortIndex = computed(() => {
    ...
    const maxSortIndex = max(indices) ?? 0;
    return maxSortIndex + 1;
  });

  const sortethPaths = computed(() =>
    sortBy(
      flatColumns.value.filter((c) => !isNil(c.sortingIndex) && !isNil(c.sortingAscending)),
      (c) => c.sortingIndex,
    ),
  );
  ...
  const rowsSorted = computed(() => {
    ...
    return orderBy(rows.value, paths, orders);
  });
